...
| 2. 完整备份（推荐） | 除 `backups/`、`cache/`、`telemetry/` 外的全部：对话、工作区、插件本体、附件、设置与凭据 | 约 245 MB |

中间不再设"对话+插件"一档：它与完整备份只差附件（约 54 MB），
而附件是对话里粘贴过的图片/文件，漏掉会让历史会话里的附件全部失效，
却没有明显提示。需要"小而勤"的备份时，选第 1 档即可。
