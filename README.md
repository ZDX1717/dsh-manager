...
插件的备份放在**插件管理**里，导出的是「当时装了哪些插件、什么版本」，约 1 KB 纯文本。
它的**主要用途是查看**：换机器或重装后照着这份清单装，而不是一键还原——
DSH 版本不同时插件未必兼容，是否重装要自己判断。

```
1. 查看清单内容        插件名@精确版本、各自声明的兼容范围、bundle 顺序、补丁层
2. 导出当前插件清单    写到 ~/.dsh/backups/
3. 从清单恢复插件      按精确版本重装（DSH 版本不同时会先警告）
4. 删除清单
```

`查看清单内容` 会把清单摊开成人能读的样子，并对比「当时 DSH」与「当前 DSH」：

```
文件：dsh_plugins_backup_20260926_012356.list
生成：2026-09-26 01:23:56
profile：web
当时 DSH：0.1.5-rc.3
当前 DSH：0.1.5-rc.3

插件（按依赖声明顺序）：
   1. @xmanrui/dsh-im@4.28.1
      声明兼容 0.1.7-alpha.1
   2. dsh-better-sidebar@0.19.1
   ...
  共 7 个
```
...
放在插件菜单而不是备份菜单的原因：插件清单只有 1 KB，
和大归档混在同一份列表里，会被「保留最近 N 个」这类保留策略顺手删掉；
而查看/恢复清单本质就是插件的事，和「安装插件」在同一处。
