...
| 档 | 内容 | 大致体积 |
|---|---|---|
| 1. 仅对话记录 | `sessions/` | 十几 MB |
| 2. 对话+插件+配置 | 再加 `storages/`、`profiles/`（含插件本体）、`settings.yaml`、凭据 | 上百 MB |
| 3. 完整备份 | 再加 `attachments/`（附件）与 `im/ integrations/ llm-*` | 更大 |

三档都排除 `cache/`、`telemetry/`；完整备份另外排除 `backups/` 自身。
恢复只覆盖、不删除目标端已有数据，所以拿"仅对话"恢复到本机不会弄丢插件；
换机器或重装后恢复，请用第 2、3 档。
