...
`8. 备份与恢复` 的备份分三种，恢复时**按文件类型自动识别**：

| 备份类型 | 内容 | 本机实测 |
|---|---|---|
| 1. 仅对话记录 | `sessions/` | 约 15 MB |
| 2. 完整备份（数据） | `storages/`、`settings.yaml`、登录凭据、`attachments/`、`im/ integrations/ llm-*`；**不含插件** | 约 60 MB |
| 3. 插件清单 | 插件名称 + 精确版本 + 兼容声明 + `cordis.patch.yml`，纯文本 | 约 1 KB |
...
恢复插件清单 = 逐条 `dsh plugin --profile web add 名称@版本`，按**精确版本**装回；
清单是纯文本，也可以照着它手动装。需要联网（registry 见 `~/.npmrc`）。
恢复数据归档不会碰插件，所以数据恢复永远不会把插件问题带回来。

三档都排除 `cache/`、`telemetry/`；完整备份另外排除 `profiles/` 与 `backups/` 自身。
恢复只覆盖、不删除目标端已有数据。
